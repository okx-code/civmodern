 ...
 *
 */
public class JourneymapNbtFileWrapper implements AutoCloseable {
    private static final Logger LOGGER = AbstractCivModernMod.LOGGER;
    ...
    public JourneymapNbtFileWrapper(Path regionFile, boolean bl) throws IOException {
        this(regionFile, regionFile.getParent(), RegionFileVersion.getSelected(), bl);
    }

    public JourneymapNbtFileWrapper(Path regionFile, Path regionDir, RegionFileVersion regionFileVersion, boolean async) throws IOException {
        this.regionFile = regionFile;
        this.version = regionFileVersion;
        if (!Files.isDirectory(regionDir, new LinkOption[0])) {
            throw new IllegalArgumentException("Expected directory, got " + regionDir.toAbsolutePath());
        } else {
            this.externalFileDir = regionDir;
    ...
    protected synchronized void write(ChunkPos chunkPos, ByteBuffer byteBuffer) throws IOException {
        ...
        int o;
        JourneymapNbtFileWrapper.CommitOp commitOp;
        if (n >= 256) {
            Path path = this.getExternalChunkPath(chunkPos);
    ...
    private JourneymapNbtFileWrapper.CommitOp writeToExternalFile(Path path, ByteBuffer byteBuffer) throws IOException {
